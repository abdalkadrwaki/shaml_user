    ...
    <div class="container mt-4 " style="width: 98%">
        <div class="card-body">
            ...
            <div class="tab-content" id="pills-tabContent">
                                ...
                                <tbody>
                                    @forelse ($friendRequests as $index => $request)
                                        <tr class="text-sm text-center text-gray-500 hover:bg-gray-100">
                                            <!-- الأعمدة الأخرى مثل الرقم واسم المكتب والرصيد بالدولار -->
                                            ...
                                            <td class="py-0.5 px-3 border-b text-center font-bold">
                                                ...
                                                <button onclick="copyBalance('balance-{{ $request->id }}')"
                                                    class="ml-2 px-2 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 transition-colors duration-300">
                                                    نسخ الرصيد
                                                </button>
                                            </td>
                                            <!-- الأعمدة الخاصة بالعملات -->
                                            @foreach ($columns as $column)
                                                @php
                                                    $columnKey =
                                                ...
                                                @endphp
                                                <td
                                                    class="py-2 px-2 border-b text-center {{ $textColor }} currency-cell">
                                            ...
                                            @endforeach
                                            <!-- عمود زر نسخ العملات لجميع الأعمدة في الصف -->
                                            <td class="py-2 px-2 border-b text-center">
                                                <button onclick="copyCurrencies(this)"
                                                    class="px-2 py-1 bg-green-500 text-white rounded hover:bg-green-600 transition-colors duration-300">
                                                    نسخ العملات
                                                </button>
                                            </td>
                                        </tr>
                ...
                <script>
                    function copyCurrencies(button) {
                        // الحصول على الصف (tr) الذي يحتوي على الزر
                        var row = button.closest('tr');
                        // الحصول على جميع الخلايا التي تحتوي على قيم العملات داخل هذا الصف
                        var currencyCells = row.querySelectorAll('.currency-cell');
                        var currenciesText = [];
                        // استخراج النص من كل خلية وإضافته إلى المصفوفة
                        currencyCells.forEach(function(cell) {
                            // الحصول على النص من الرابط داخل الخلية
                            var text = cell.innerText.trim();
                            if(text) {
                                currenciesText.push(text);
                            }
                        });
                        // دمج النصوص مع فاصلة أو أي فاصل مناسب
                        var finalText = currenciesText.join(' | ');

                        // استخدام Clipboard API لنسخ النص إلى الحافظة
                        navigator.clipboard.writeText(finalText)
                           .then(function() {
                               alert("تم نسخ العملات: " + finalText);
                           })
                    ...
                    // مثال لدالة نسخ الرصيد (إن لم تكن موجودة بعد)
